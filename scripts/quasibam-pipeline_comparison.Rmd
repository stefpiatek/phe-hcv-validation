...
for(item in 2:length(samples)){
  ...
  pipeline_qb <- read_delim(file = here("data", glue("{sample}_{pipeline}_quasibam.txt")),
    ...
    mutate(RefN = str_to_upper(RefN)) %>%
    filter_to_roi() %>%
    # sample 18 is out of sync with it's reference for all samples by 1
    ...
    mutate(Pos = if_else(sample_name == "170908_3" & pipeline == "vicuna_bwa" & 
                           region_name == "NS5A" & between(Pos, 1163, 1170),
                         -1,
                         as.double(Pos))) %>%
    mutate(Pos = if_else(sample_name == "170908_14" & pipeline == "vicuna_bwa" & 
                           region_name == "NS5A" & between(Pos, 1331, 1342),
                         -1,
                         as.double(Pos))) %>%
    mutate(Pos = if_else(sample_name == "170908_18" & pipeline == "vicuna_bwa" & 
                           region_name == "NS5A" & between(Pos, 1164, 1173),
                         -1,
                         as.double(Pos))) %>%
    filter(Pos > 0) %>%
